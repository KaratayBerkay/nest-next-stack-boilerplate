...
1. **[`/pricing`](./pricing/screen.md)** — top-level route, redirects immediately to step 2.
   **Currently broken for a logged-out visitor**, same net effect as web though a different
   mechanism — see [CROSS-029](../issues.md#cross-029).
...
3. **[`v1/checkout`](./v1/checkout/screen.md)** — payment collection or plan change. Unlike web,
   ...
   mutation with no intermediate BFF validation bug) — see [CROSS-030](../issues.md#cross-030) for
...
4. **[`v1/settings/billing`](./v1/settings/billing/screen.md)** — ongoing subscription management:
   ...
   web's here — see [CROSS-034](../issues.md#cross-034)), cancel.
...
permanently-zero growth stats — [MOB-023](../issues.md#mob-023) — and 7 of its 8 source files are
dead code duplicating the live implementation — [MOB-022](../issues.md#mob-022).
