...
- **2026-07-24 — Phases B–D implemented (login + all auth pages, AuthLayout, LabeledField, LoginResult, server fixes, orphan cleanup).** Generated code across 13 files: `AuthLayout`, `LabeledField`, `LinkText`, updated `Input` with 5 new params, sealed `LoginResult` types, 202/MFA handling in `LoginServer.call` + `mfaToken` fix in `MfaServer.call` + `loginMfa` in `LoginActions`, complete login-page rebuild (form/MFA/signed-in states, client-side validation, BFF error mapping, locale-aware nav), register page rebuild (direct session from register response), forgot-password (card-swap on success), reset-password (token-passthrough + timer-redirect), verify-email (auto-fire + success/failure states). Deleted 12 orphaned auth files + `/auth/mfa` route. Clean: `flutter analyze` 0 issues, `dart format` clean, `flutter test` 300/300 green. Phase A verified already-done (main.dart, pubspec, Makefile, README, .env.example all correct). Phase C-L5 (widget tests) and Phase E (whole-app parity) remain.
