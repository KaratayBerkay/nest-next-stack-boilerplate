...
- [ ] **Make the viewport render the queue.** `ToastViewport` consumes
...
- [ ] **One provider, app-level.** Delete the nested
...
- [ ] **Tokens.** Replace `toast.tsx:36-40` hardcoded palette with
      ...
      all four themes.
- [ ] **Close ui-upgrade A7 while the file is open** (it's why two D3
...
- [ ] **Tests + demo acceptance:** unit test — render provider+viewport,
...
- [ ] Rebuild the popup on **our Popover** (`Popover`, `PopoverTrigger`
...
- [ ] Desktop collision handling in PopoverContent (shared win): flip
...
- [ ] **Jump navigation:** `DatePicker` accepts and forwards
...
- [ ] **`picker` prop:** `picker?: "day" | "month" | "year"` (default
      `"day"`).
...
- [ ] Wire F1 messages + variants through the new panel path (F9 landed
      the trigger; keep it).
- [ ] Demos (this resolves the ⧗V7-format deferrals from ui-upgrade-2 D3):
...
- [ ] **Fix the base control:** keep the native `<input type="checkbox">`
...
- [ ] **CheckboxCard** (the non-conventional ask, shadcn-blocks' choice
...
- [ ] **CheckboxChip** (filter-chip style): rounded-full pill toggling
      `bg-brand text-brand-fg` when checked, with optional count and ×
      affordance — for the "Board Filters"-type scenarios.
- [ ] Demos: rebuild `/v1/en/ui/checkbox` tabs — "Terms Consent" (base,
...
- [ ] **`FileUpload`** (`src/components/ui/file-upload/`): a dropzone
...
- [ ] **`ImageUpload`** (`src/components/ui/image-upload/`): builds on
...
- [ ] Two new demo pages + routes (`file-upload`, `image-upload`),
...
- [ ] Skill-doc: add both to the `ui-components` SKILL.md component list
...
- [ ] Implement + demo readout; test asserts children remain in the DOM
...
- [ ] Implement merge + add the demo tab content + test (child onClick
...
- [ ] Restructure DialogContent: `<dialog>` keeps `flex flex-col
      ...
      (`px-6 pt-6` / `px-6 pb-6`).
- [ ] Because scrollbars are hidden app-wide, DialogBody gets the
      **scroll-fade treatment** (S4): CSS mask fading top/bottom edges
      when scrollable — the affordance replacing the scrollbar.
- [ ] Demo: the missing **"Terms Scroll"** tab (long ToS body; title +
...
- [ ] Fix by destructuring `onClick`/`onKeyDown` out of props and
      composing them inside the internal handlers (consumer handler runs
      first, then close+refocus unless `e.defaultPrevented`).
- [ ] Audit the same pattern across select-item, context-menu, menubar,
...
- [ ] Test: item with a consumer `onClick` still closes the menu and
...
- [ ] Recount the `useComponentVariant` roster (currently **21**: the 18
...
- [ ] Replace SKILL.md's demo-page sentence ("showing every variant/size")
...
- [ ] Fix ui-upgrade-2 F10 bullet-1's wrong "N/A" note (it claimed the
      roster was complete at 18 while card/command/kbd already consumed).
- [ ] One-line doc comment on `drawer.tsx:6` (`Drawer =
...
- [ ] **Migrate the three untouched pages** — command, popover,
...
- [ ] **De-shell select and input**: dissolve `SelectDemo`/`InputDemo`
...
- [ ] **Delete all eight `*Demo.tsx` leftovers** (command, dialog,
...
- [ ] **Kebab-case scenario ids sweep**: every `UIExample.id` becomes the
      scenario slug (`"single-state"`, not `"components"`). Gate:
      `grep -rn 'id: "components"\|id: "examples"' src/views/ui/` → empty.
- [ ] **Ship the missing named tabs** flagged in the D3 downgrades:
...
- [ ] **Token sweep of the ~11 offender demo files** (alert-dialog:40,
...
- [ ] **logo-spinner anatomy**: move `LogoSpinner.tsx` →
...
- [ ] Flip ui-upgrade.md's `[~]`/`[ ]` rows that F-items completed
      (A2/F2, A5/F4, A6/F3, A9/F1, G1-stray/F9, T1/F12, A1-nit/F13),
      citing ui-upgrade-2 §F — the never-executed instruction.
- [ ] As Part C/U items land, flip the corresponding ui-upgrade-2
...
- [ ] Write the rules above into `globals.css` comments (next to the
...
- [ ] **Component palette purge** (the five offenders + token misuse):
...
- [ ] **button** (`button-styles.ts`) — align every variant to the
...
- [ ] **icon-button** — square sizes matching the field scale (`size-8/9/10`),
      `rounded-md` (today bare `rounded`), same focus/disabled standard.
- [ ] **toggle / toggle-group** — on-state today is `bg-surface` (near
...
- [ ] **counter** — restyle as one joined control (btn–value–btn inside a
...
- [ ] **slider** — thumb `size-4` → `size-5` (touch), `hover:scale-110
...
- [ ] **input-otp** — slots `h-10 w-9 rounded-md`; **active slot** gets
...
- [ ] **input-group** — addons `bg-surface text-muted border-border`,
...
- [ ] **time-input** — `h-10` → h-9, wrapper `rounded-lg` → conform;
      ...
      style.
- [ ] **combobox** — trigger conforms to select trigger; list panel
...
- [ ] **select** — trigger `rounded` → `rounded-md`; content/items adopt
...
- [ ] **calendar** (`calendar.tsx:35-66`) — **today** is `bg-surface`
      ...
      Rides U2/U3.
- [ ] **date-picker** — fold the floating "selected value + clear" box
...
- [ ] **dialog** — rides C4 (DialogBody + pinning + scroll-fade); visual:
...
- [ ] **hover-card** — `w-64 rounded-md shadow-md` → `w-72 rounded-lg
      shadow-lg p-4` + scale-in; it hosts rich content, so it sits a step
      *above* tooltip in the ladder.
- [ ] **toast** — rides U1; visual spec: `rounded-lg border bg-bg
...
- [ ] **tabs** — default list `bg-surface rounded-lg p-1` + active
...
- [ ] **breadcrumb** — separators `text-muted/60 size-3.5`; current page
      `text-fg font-medium` (`aria-current` styled, not just semantic);
      collapse `…` trigger = icon-button ghost hover.
- [ ] **pagination** — active page is `bg-surface border-fg
...
- [ ] **navigation-menu** — trigger chevron `transition-transform
      data-[state=open]:rotate-180`; viewport panel `rounded-lg border
      shadow-lg` conform; motion tokens already wired (`data-[motion…]`)
      — keep.
- [ ] **menubar** — bar `shadow-sm` → `shadow-xs`; menus adopt the V3
      menu spec wholesale.

### V5 — Data display

- [ ] **card** — variant cleanup: `default` `rounded-xl border shadow-xs`,
...
- [ ] **table** — wrap in `rounded-lg border border-border` shell with
...
- [ ] **badge** — rationalize the map (today `default` and `secondary`
...
- [ ] **avatar** — size map `xs size-6 / sm size-8 / md size-10 / lg
      ...
      tokened ✓.
- [ ] **accordion** — trigger `py-4 text-sm font-medium hover:text-brand
...
- [ ] **separator** — token ✓; add `label` slot variant (`text-xs
      text-muted bg-bg px-2` centered over the line) for the "OR divider"
      demo.
- [ ] **skeleton** — upgrade pulse → shimmer sweep (`bg-surface-hover`
      ...
      preset).
- [ ] **empty** — structure the anatomy: icon in `size-12 rounded-full
...
- [ ] **kbd** — add key-cap depth `shadow-[inset_0_-1px_0_var(--border)]`,
      `min-w-5 justify-center text-xxs`; sequence separator `text-muted`.
- [ ] **typography** — `tracking-tight` on h1–h3; blockquote border
      `border-brand/30`; inline code chip ✓; lists `marker:text-muted`;
      lead paragraph style `text-lg text-muted`.
- [ ] **aspect-ratio** — no visual surface; N/A (record and skip).
- [ ] **carousel** — nav buttons move inside the frame (`left-2/right-2`)
      ...
      the strip (S3).
- [ ] **collapsible** — keyframes ✓; adopt accordion's chevron-rotation
      convention on the demo trigger so the two disclosure components
      read identically.
- [ ] **scroll-area** — thumb `bg-border` → `bg-border hover:bg-muted/60
...
- [ ] **resizable** — fix dead focus (`ring-ring` → `ring-brand`, V0);
...
- [ ] **progress** — indicator `transition-transform duration-500
...
- [ ] **spinner** — size map matched to button sizes (`size-3.5/4/5`),
      inherits `currentColor` ✓; becomes the single spinner used
      app-wide (S3 dedup — button's private SVG at `button.tsx:9-33`
      folds in).
- [ ] **alert** — token variants via V0 purge; each status variant gets
...
- [ ] `src/views/ui/_shared/VariantGallery.tsx`: given a render prop
...
- [ ] Add a **"Variant Gallery" last tab to every roster page** (21 today,
...
- [ ] **Auth block** → input page: "Login Card" (email+password, F1
      errors, remember-me Checkbox, submit Button with C1 loading).
- [ ] **Settings block** → switch page: "Notification Settings" panel
      (grouped switch rows + Save bar) replacing the bare rows demo.
- [ ] **Pricing block** → card page: upgrade "Pricing Tiers" to a real
      3-tier block with feature checklists (Kbd-free ✓ list), highlighted
      tier, CTA buttons.
- [ ] **Invoice/table block** → table page: "Invoice" (caption, right-
      aligned amounts, footer total row) as the canonical real-world
      table.
- [ ] **Team management block** → avatar + dropdown pages: member rows
      (AvatarGroup, role Badge, row-overflow DropdownMenu — dogfoods C5).
- [ ] **Checkout summary block** → drawer page: upgrade "Cart Summary"
      with line items, totals, promo-code InputGroup.
- [ ] **Profile form block** → image-upload page: avatar upload + name
      fields + save (composes U5 + F1 + C1).
- [ ] **Command-palette block** → command page (rides the C8 migration):
      ⌘K dialog with groups, Kbd shortcuts, empty state.
- [ ] **Notifications block** → toast page (rides U1): the "Notification
      System" scenario becomes an inbox-style block whose actions fire
      real toasts.
- [ ] **Stats block** → progress/badge pages: KPI tiles with Progress +
      trend Badges (token-only, dataviz-skill palette rules if charts
      ever join).
- [ ] Each block: fits viewport, token-only, controlled state with
...
- [ ] **`scroll-fade` utility** (shadcn, June 2026): a CSS mask utility
...
- [ ] **Spinner/loading consistency**: one Spinner component everywhere a
...
- [ ] Evaluate (decision recorded here, yes/no each): chat-interface
...
- [ ] Playwright smoke (webapp-testing skill): walk all 60
      ...
      tab clickable, one interaction per page's first tab (click the
      toast button and *assert a toast appears* — U1's regression test;
      open the date picker at 360px and assert no horizontal overflow —
      U2's).
- [ ] axe pass per page (same run), gated in CI next to the contrast
      `--strict` step in `frontend-ci.yml`.
- [ ] Visual eyeball checklist per family in light+dark recorded in this
      doc when each family lands (the ui-upgrade-2 D4 step that was
      skipped).
...
1. **V0 design-language foundation** — the rules, the `useExitAnimation`
   hook, the component palette purge (alert, confirm-dialog, textarea,
   switch, drawer, resizable), the new grep-gates. Small diff, everything
   after conforms to it.
2. **U1 toast** (app-wide dead feature, worst user-facing) — with its A7
   rider, V3's toast visual spec, and the S2 notifications block.
3. **U2 + U3 date-picker** (responsive panel first, then picker modes) —
   V2's calendar/date-picker polish and the C-items in the same files
   ride along (popover title prop, collision helper, shared mobile
   sheet).
4. **U4 checkbox** family (base glyph → cards/chips → demos) — V2's
   radio/switch/label polish rides (same family, same review).
5. **U5 uploads** (new components built to V0 spec from day one; S2
   profile-form block).
6. **C1–C7** defect/debt batch (button, dialog, dropdown, gates, docs) —
   V1 button/icon-button polish and V3's dialog/menu-spec ride here (C4
   and C5 already open those files).
7. **C8 migration completion + S1 VariantGallery + S2 blocks + remaining
   V-family polish**, family-by-family (forms → overlays → navigation →
   data → feedback), commit per family — each family commit = content
   (C8/S2) + skin (V) + gallery (S1), eyeballed once per V-acceptance;
   C9 doc flips ride every commit.
8. **S3 pattern adoptions** (scroll-fade lands with C4; spinner dedup
   with C1; the rest as decided).
9. **S4 smoke + axe in CI** once ≥ half the families are done; it then
   guards the rest.
