...
- **Phase 0 — Key infrastructure only, messages still plaintext. ✅ COMPLETE**, with one authorization gap found in the 2026-08-03 security review and fixed: `claimBundle` had no friendship check (item 5 there). Redis key design: `e2ee:bundle:<deviceId>`, `e2ee:otpk:<deviceId>`, `e2ee:active-device:<userId>` (§2.1), all session-TTL'd via `TokenStoreService.extendTTL()` and explicitly cleared on logout. New `src/e2ee/` module with register/claim/status/replenish endpoints and the `LPOP`-based atomic claim. Frontend: `lib/crypto/primitives.ts|types.ts|store.ts|identity.ts|fingerprint.ts`, `useE2eeIdentity()`, the three-layer key-registration API files.
- **Phase 1 — Handshake + ratchet engine. ✅ COMPLETE and now security-reviewed** — see both the 2026-08-03 re-verification (DH-ratchet never fired, X3DH OTPK path broken) and the 2026-08-03 security review below (a replayed/re-viewed message could corrupt a live session; an unbounded skip-ahead loop was a DoS vector). All fixed, all covered by regression tests specifically written to catch each one.
- **Phase 2 — DM encryption goes live. ✅ COMPLETE**, subject to the same Phase 1 caveats — the plumbing (envelope threading, leak fixes, feature flag, blocking UI) was always correctly wired; it was the crypto underneath it that didn't work until the fixes in both review passes below. Also gained an envelope size cap (security review item 8) on both the DTO and WS paths.
- **Phase 3 — Room membership schema (prerequisite; rooms still plaintext). Schema and durable join/leave are COMPLETE and correct** (`Room`/`RoomParticipant`, seeded with today's 5 room slugs, `persistJoin`/`persistLeave` genuinely wired into the WS gateway). **The "DB-backed `isValidRoom()`" claim was false as of 2026-08-03** — `isValidRoom()` was still the original hardcoded `CHAT_ROOMS` array check, never querying `Room` — now fixed (a background-refreshed cache of `Room.slug`, since the WS hot path needs `isValidRoom()` to stay synchronous). Migration `20260803000000_add_room_membership` applied.
- **Phase 4 — Room encryption goes live. The schema/controller/service existed, but "`ChatRoomHandlers.tsx` rotates/distributes before sending" was false as of 2026-08-03** — no code anywhere called `rotateSenderKeyChain`/`wrapSenderKey`/the publish endpoint, and the local chain-key storage key used by the sender (`roomId` alone) didn't even match what the receive path looked up (`` `${roomId}:${senderId}` ``), so every encrypted room message was undecryptable by every recipient, unconditionally. Now actually wired end-to-end. The 2026-08-03 security review then found and fixed two real vulnerabilities in that new wiring before it shipped further: the rooms controller rejected the real room slug (validated `:roomId` as a UUID), and `publishSenderKeys` trusted a client-supplied `senderDeviceId` instead of the authenticated session — allowing device impersonation and a pre-emptible denial-of-service against a targeted member's key distribution. A narrow two-tab rotation race is documented but not fixed (see that section). Migration `20260803000001_add_room_sender_key_distribution` applied.
- **Phase 5 — Attachment encryption. ✅ COMPLETE, but was silently broken for any real-sized file until the 2026-08-03 security review**: `toBase64()` spread the full ciphertext byte array as function arguments, which throws past ~64-128KB — essentially guaranteed for any real photo or document, despite §4 explicitly expecting megabyte-sized attachments. Every existing test used few-byte inputs and never caught it. Fixed with chunked conversion; a new ~293KB round-trip regression test pins it. `EncryptedAttachmentPreview.tsx`'s object-URL lifecycle was also reviewed and is correct (properly revokes on unmount/re-run, including the in-flight-decrypt-then-unmount race).
- **Phase 6 — Hardening. Wiring was complete, but the safety-number feature itself was cryptographically broken until the 2026-08-03 security review** (`SafetyNumberModal.tsx`/`SafetyNumberBadge.tsx` were genuinely imported/rendered in `ChatViewHeader.tsx` and `ChatRoomSidebar.tsx`; `POST /api/e2ee/keys/wipe`; `docs/backend/E2EE.md` and `docs/adr/006-e2ee-chat-protocol.md` written — all of that checked out). What didn't: the fingerprint encoding wasn't injective and the DM safety-number combiner ran both fingerprints through a 32-bit non-cryptographic hash, trivially brute-forceable — meaning the one feature that exists to catch a compromised server substituting someone's identity key provided essentially no real protection against exactly that attack. Fixed (see that section's item 4); `GET /api/e2ee/keys/identity/:userId` also extended to return `identityAgreementKey` (previously signing-key only), needed for the Phase 4 room-wrap fix. Signed-prekey rotation (~30d, §1.2) remains unimplemented — documented as a deferred defense-in-depth gap, not an active vulnerability.

### 2026-08-03 independent re-verification — critical bugs found and fixed

Berkay asked for the "✅ COMPLETE" claims above to be checked. They were not accurate. Four bugs in the DM crypto path and one structural gap in room encryption meant the feature did not actually provide the security properties (or in the room case, the basic functionality) the plan claims — despite `docs/backend/E2EE.md`, the ADR, and 100/100 passing crypto tests at the time. All of the following are now fixed, tested, and gate-clean; see the register memory for this investigation for more detail than fits here.

1. **DM Double Ratchet never performed a DH-ratchet step** (`lib/crypto/ratchet.ts`). `initSenderSession` seeded `peerDhPub: ""`, and the only branch that could ever update it required it to already be truthy — a chicken-and-egg deadlock, so the initiator's side could never detect the peer's ratchet key changing. Both `sendingChainKey`/`receivingChainKey` were also seeded to the literal same X3DH output on both sides. Net effect: Alice's k-th sent message and Bob's k-th sent message used the *identical* symmetric key, for every k, forever — no post-compromise recovery, the entire stated reason (§1.4) for building a full Double Ratchet instead of a simpler hash ratchet. **Fixed**: each direction's chain is now seeded via a real DH (initiator against the peer's signed prekey, mirrored by the responder for DH-symmetry), `peerDhPub`/`sendingChainKey`/`receivingChainKey` are properly nullable, and a side's own sending chain is only ever (re-)established lazily via a fresh DH-ratchet right before it next sends. The skipped-message-key cache was also made epoch-aware (keyed by `(dhPub, index)`, matching Signal's actual design) after a new regression test caught that a message delayed across a ratchet boundary couldn't be recovered under the old plain-numeric-index cache.
2. **X3DH one-time-prekey path dropped `dh4` on the responder side** (`lib/crypto/chat.ts`, `x3dh.ts`). The function that assembled the responder's private keys declared but never populated `oneTimePrekeyPrivateKey`, and `x3dhInitiate` fabricated `usedOneTimePrekeyId` as a `Date.now()` placeholder instead of the real claimed key's id. Since `identity.ts` generates and uploads 10 real one-time-prekeys per device by default, consuming one is the *common* first-contact path, not an edge case — meaning the first message of essentially every new DM conversation failed to decrypt. **Fixed**: the real claimed prekey's id now rides in the X3DH preamble, and the responder looks up its matching local private key by that id before calling `x3dhRespond`.
3. **`decryptDmMessage`'s AAD didn't match what `encryptDmMessage` built** (`lib/crypto/envelope.ts`). Encrypt used `(senderDeviceId, recipientUserId)`; decrypt passed `senderUserId` for *both* positions (a `// recipient is us` comment revealed the mistaken assumption). This alone would have failed every single decrypt via the real production call path with an AEAD auth-tag error, independent of bugs #1/#2 — caught by a new test that exercises `encryptDmMessage`/`decryptDmMessage` directly instead of only the lower-level ratchet functions with manually-matched arguments. **Fixed**: `decryptDmMessage` now takes the current user's own id as an explicit parameter and uses it for the AAD's recipient position, matching what `encryptDmMessage` builds.
4. **`decryptDmMessage` was initializing the receiver ratchet session with the wrong public key** (`envelope.ts`). It passed `envelope.x3dhInit.ephemeralKey` (the X3DH ephemeral key) where `envelope.header.dhPub` (the actual per-message ratchet key) belongs — two different keys. **Fixed**: uses `header.dhPub`, matching what every message header actually carries.
5. **Room sender-key distribution had no frontend wiring at all.** `rotateSenderKeyChain`/`wrapSenderKey`/`unwrapSenderKey` and the backend's publish/fetch sender-keys endpoints had zero callers anywhere in the frontend; the sender's local chain was stored under `roomId` alone while the receive path looked up `` `${roomId}:${senderId}` `` — a key format nothing ever wrote to. **Fixed**: `distributeSenderKeyIfNeeded()`/`ensureReceivedSenderKey()` (new, `lib/crypto/sender-keys.ts`) actually call the room-members/publish/fetch endpoints (new `api/server/e2ee/{room-members,publish-sender-keys,fetch-sender-keys}.ts`), wired into `ChatRoomHandlers.tsx` (send) and `query.ts`/`event-dispatch.ts` (receive). Per-recipient wrapping uses a real X25519 DH between both parties' stable identity agreement keys (HKDF'd with a distinct domain-separation label) rather than an undefined "simplified" key — fetched via the existing *non-consuming* identity/bundle-status endpoints, so distributing or receiving a room key never spends a one-time-prekey.
6. The backend test suite was not gate-clean: 5 of 60 jest suites were failing (mostly pre-existing tests whose mocks/assertions were never updated for the new `envelope`/`encrypted` params or the `hget`→`hmget` change) — fixed. `messaging-room.service.ts`'s `isValidRoom()` false claim (item above) also fixed. Full suites now: backend 59/60 (`device.service.spec.ts` fails for an unrelated, pre-existing reason — `MAX_DEVICES_PER_USER`/`device.count`, not part of this feature); frontend 60/62 test files (`PageContent.test.tsx` for the settings/security page fails for an unrelated, pre-existing `ThemeProvider` wrapping reason). TypeScript clean on both apps; `nest build` clean.

### 2026-08-03 security review (second pass) — carefully re-checked, more issues found and fixed

Berkay asked for a full, careful security/logic pass over the whole mechanism (not just re-confirming the fixes above) before marking anything complete. This found real issues in code that was *not* touched by the first pass, plus one issue in the fix that was just made. All of the following are fixed, tested, and gate-clean unless explicitly marked as a deferred/documented limitation.

1. **A replayed or merely re-viewed old message could corrupt a live DM ratchet session** (`envelope.ts`, `ratchet.ts`). A conversation's first stored message permanently carries `x3dhInit` in its envelope; re-fetching/re-rendering history (pagination, a cache refetch, a page reload) decrypts it again, and `decryptDmMessage` unconditionally re-ran the X3DH bootstrap whenever it saw `x3dhInit` — silently rolling back an already-advanced session, breaking every message after it. **No attacker required** — normal history browsing triggers it. Fixing only that surfaced a deeper issue: even without re-running X3DH, feeding `ratchetDecrypt` any envelope whose `header.dhPub` no longer matches the current epoch (any old, already-consumed message) was indistinguishable from a genuine new ratchet key, so it would perform a bogus DH-ratchet and overwrite `rootKey`/`receivingChainKey`/`peerDhPub` with garbage before the AEAD tag check ultimately failed. **Fixed properly**: `ratchetDecrypt` was restructured to compute everything into local scratch variables and only commit to the session object *after* `xchachaDecrypt` has actually authenticated the ciphertext — so a stale/replayed/corrupted envelope can never touch live session state, regardless of what the storage layer does on read. `decryptDmMessage` also now skips X3DH bootstrap entirely whenever a session already exists. Caught by a new regression test that decrypts old and new messages out of sequence and asserts the session keeps working.
2. **Unbounded skip-ahead loop — a DoS via a single wire-controlled field.** Both `ratchet.ts`'s skipped-message-key derivation and `sender-keys.ts`'s room-chain fast-forward looped from the current position to a peer-supplied index (`header.n`/`header.pn`/`envelope.chainIndex`) with no upper bound — none of these fields are authenticated until *after* the loop completes. A malicious or corrupted message with an enormous index would force a synchronous, tab-freezing HMAC loop. **Fixed**: both now cap the jump distance and throw rather than loop unboundedly.
3. **`toBase64()` crashes on any real attachment.** `btoa(String.fromCharCode(...data))` spreads the entire byte array as individual function arguments — every JS engine throws (`RangeError`, "too many function arguments" / stack size exceeded) somewhere around 64K-128K elements. Attachments are explicitly expected to be megabytes (§4) — comfortably over that threshold for essentially any real photo or document. Every existing attachment test used inputs of a few bytes, so this never surfaced. **Fixed**: chunked conversion (8KB at a time) in `primitives.ts`, reused by `attachments.ts` in place of its own ad-hoc unsafe conversion. New regression test round-trips a ~293KB file.
4. **The safety-number feature (§1.6) provided no real cryptographic assurance.** Two independent bugs: (a) `computeUserFingerprint`'s hex-to-decimal encoding mapped each hex nibble to its decimal value, but nibbles 0-9 emit one digit while a-f emit two — so the encoding is not injective (hex nibbles `[0xa, 0x1]` and `[0x1, 0x0, 0x1]` both render as `"101"`) and the output had to be zero-padded to a fixed length, meaning most displayed digits were constant padding rather than actual key material. (b) `computeDmSafetyNumber` combined both parties' fingerprints through a hand-rolled 32-bit non-cryptographic hash (`((hash << 5) - hash + char) | 0`) before display — a ~4.3 billion-value output space that's trivially brute-forceable, meaning an attacker who has substituted a peer's identity key (exactly the attack §1.6 exists to catch) could search for a fake key that renders the *same* displayed safety number. Together these meant a diligent user comparing safety numbers got essentially no protection against the compromised-server substitution attack this feature is the plan's stated defense against. **Fixed**: fingerprints now use an injective, entropy-preserving encoding (5 decimal digits per 16-bit chunk, Signal's own approach); the DM safety number is now a plain concatenation of both fingerprints in canonical order (matching what §1.6 actually describes) rather than a lossy re-hash. New `fingerprint.test.ts` (previously nonexistent) pins both properties.
5. **`POST /api/e2ee/keys/bundle/:userId/claim` had no friendship check.** Any authenticated user — not just friends — could claim any other user's bundle, consuming one of their scarce one-time-prekeys, even though DM sending itself is friend-gated and a non-friend could never actually use the resulting session. This let any user grief another's OTPK pool for a conversation they could never have. **Fixed**: gated behind the same `FriendsService.areFriends()` check the DM send path already enforces.
6. **`POST /api/e2ee/rooms/:roomId/sender-keys` (publish) took `senderDeviceId` from the request body instead of the authenticated session.** Any authenticated user could publish wrapped sender-key rows *claiming to be* another member's device, for any room, any epoch. A forged wrapped blob can't itself be decrypted (it can't be validly encrypted without the real DH secret), but because publishing uses `createMany({skipDuplicates: true})` against a `(roomId, senderDeviceId, epoch, recipientDeviceId)` unique constraint, an attacker who can observe a target's `deviceId` (visible in the clear on every message envelope that member sends) could *pre-emptively* publish a garbage row for a future epoch+recipient pair — permanently and silently blocking the real key from ever landing for that recipient once the real rotation happens, since the real insert would be silently skipped as a duplicate. This is a real, exploitable denial-of-service against room-message availability for a targeted member. **Fixed**: `senderDeviceId` is now always derived from `user.deviceId` (the authenticated session), matching how the adjacent `fetchSenderKeys` endpoint already did it correctly. Also added proper `class-validator` decorators to the publish DTO (previously undecorated — accepted any shape).
7. **The E2EE rooms controller validated `:roomId` as a UUID while the frontend only ever addresses rooms by slug.** `publishSenderKeys`/`fetchSenderKeys`/`getRoomMembers` all rejected the real slug (e.g. `"general"`) with a 400 before reaching the service — these endpoints were unreachable as designed by any real caller. **Fixed**: controller now accepts the slug; the service resolves slug→`Room.id` internally (`getRoomMembers` was already doing this on the read side, just inconsistently with the other two).
8. **No size limit on the E2EE envelope field, in either the REST/GraphQL DTOs or the WS path.** `envelope` was validated only as "is an object" — a client could send an arbitrarily large blob straight into an unbounded Postgres `JsonB` column and every connected tab's realtime relay. **Fixed**: a `class-validator` constraint capping serialized size at 32KB (a real envelope is ~8-10KB at the 5000-char plaintext cap) on both DTOs, plus an equivalent hand-written check in `messaging-ws.gateway.ts` — WS frames bypass the DTO/ValidationPipe pipeline entirely, so the REST/GraphQL-only fix would have left the primary live-chat path completely uncovered.
9. **Fixing #5 introduced a real NestJS module dependency cycle that would have crashed the backend at boot** (`E2eeModule → FriendsModule → AuthContractsModule → E2eeModule`). Caught by actually attempting to boot the app (`nest start`), not just `tsc`/`nest build` — a circular-dependency error is a runtime module-graph concern that a type-check can't see. Fixed with `forwardRef()` on both new edges (`e2ee.module.ts` and `friends.module.ts`).

**Deferred, documented rather than fixed:**
- **A narrow two-tab race in room sender-key rotation.** `distributeSenderKeyIfNeeded` reads-then-writes IndexedDB with no lock; two tabs rotating at nearly the same moment could each publish a different chain key for "the same" epoch to different recipients (whichever request reaches the backend's unique-constraint insert first per recipient wins), leaving some recipients unable to decrypt that sender's subsequent messages until the next rotation. Availability-only (not a confidentiality issue), requires near-simultaneous multi-tab sends to trigger. Not fixed given its narrow scope — a real fix would want to piggyback on `useRealtimeCoordination.ts`'s existing leader-tab election rather than adding an independent lock.
- **Signed-prekey rotation (§1.2: "rotated ~30d") was never implemented.** `identity.ts` generates one SPK on first use and reuses it forever (`getCurrentSignedPrekey()` only ever looks at `keyId: 0`). This only matters if a device's SPK private key is later compromised (it would extend the blast radius of that compromise across all past/future first-contacts instead of just ~30 days of them) — defense-in-depth, not an active vulnerability, but a real gap versus the stated design.
- **No durable persisted-plaintext cache for DMs.** The Double Ratchet's forward secrecy means each message's key is used once and discarded — this is correct and intentional. But this app currently re-decrypts from the raw envelope on every fetch (React Query's `staleTime: Infinity` on DM history avoids this within a single tab session, but not across a page reload, a new tab, or any explicit cache invalidation). Once the ratchet has moved past a message, re-decrypting it will fail. Real Signal/WhatsApp avoid this entirely by decrypting once on receipt and persisting the *plaintext* locally (e.g., in IndexedDB), never re-deriving from ciphertext. This app does not yet do that. This is an architectural product decision (retention policy, and it does mean plaintext would live on disk unencrypted, which changes the "device loss = key loss" story in §6) rather than something to silently fix mid-review — flagging it here for an explicit call before it surprises someone as "my old messages disappeared after I refreshed."
- **`RoomSenderKeyDistribution` rows are never cleaned up** for old epochs. Not a security issue (an old epoch's wrapped keys don't grant access to anything beyond what that epoch's members already had), just unbounded storage growth over time — worth a periodic cleanup job eventually.

### What remains — verification gaps

The following verification work from §7 is still TODO:
...
4. ~~**Envelope size cap** on the backend DTO~~ — **✅ DONE** (2026-08-03 security review, item 8 above): enforced on both the REST/GraphQL DTOs and the WS path, since WS bypasses the DTO pipeline entirely.
