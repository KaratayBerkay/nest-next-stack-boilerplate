...
export class OAuthController {
   ...
   * Query params:
   *   state — CSRF token (generated by the BFF / mobile app, required)
   *   redirect_uri — where NestJS should redirect back after OAuth (BFF callback URL or app scheme)
   *   code_challenge — S256 challenge of a verifier the initiating client keeps to itself;
   *     required for app-scheme redirect URIs, see OAuthService.buildAuthUrl (CROSS-032)
   */
  @Get(':provider')
  @Redirect()
  async initiate(
    ...
    // An absent or empty query param both mean "no challenge registered" —
    // the service decides whether that is acceptable for this redirect_uri.
    const clientCodeChallenge =
      codeChallenge === undefined || codeChallenge === ''
        ? undefined
        : codeChallenge;
    const url = await this.oauth.buildAuthUrl(
      provider,
      state,
      redirectUri,
      clientCodeChallenge,
   ...
   * back to the registered redirect URI with ?state=..&claim=.. so the
   * BFF / mobile app can redeem the profile via loginWithOAuth. The claim
   * is minted per completed handshake and exists only on this redirect —
   * see OAuthService.handleCallback (CROSS-032).
   */
  ...
  async callback(
    ...
    try {
      const { redirectUri, claim } = await this.oauth.handleCallback(
        code,
        state,
      );
      ...
      return res.redirect(
        HttpStatus.FOUND,
        `${safeTarget}${separator}state=${encodeURIComponent(state)}&claim=${encodeURIComponent(claim)}`,
      );
    } catch (err) {
