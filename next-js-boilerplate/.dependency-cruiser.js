...
module.exports = {
  forbidden: [
    {
      // Bumped from the generated default's 'warn' to 'error': this is the CI gate audit
      // item #10 asked for, and a warn-only circular-dependency rule doesn't gate anything.
      name: "no-circular",
      severity: "error",
      comment:
    ...
    {
      // Codifies the lib/ -> features/ import-direction fix (audit item #12): lib/ is meant
      // to be feature-agnostic, so features/ should depend on lib/, never the other way round.
      name: "no-lib-importing-features",
      severity: "error",
      comment:
        "src/lib/ is meant to be feature-agnostic shared code; a module here importing from " +
        "src/features/ inverts the intended dependency direction. Move the shared type/util " +
        "into src/lib/ or src/types/ instead.",
      from: {
        path: "^src/lib/",
      },
      to: {
  ...
  options: {
    ...
    enhancedResolveOptions: {
      ...
      // Not just .ts/.tsx: dependency-cruiser also needs to resolve plain-JS files inside
      // node_modules (e.g. next/server.js, next/headers.js) even though it won't follow
      // into them (see doNotFollow below) — an extensions list scoped to our own source
      // types leaves those edges unresolvable and produces false-positive violations.
      extensions: [".tsx", ".ts", ".d.ts", ".jsx", ".js", ".mjs", ".cjs"],
...
// generated: dependency-cruiser@18.0.0 on 2026-07-12T19:22:08.039Z
