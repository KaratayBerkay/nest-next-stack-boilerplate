...
describe("deviceLabel", () => {
  ...
  it("labels a server-side/API call as 'API client', matching the literal 'node' UA this backend sends", () => {
    expect(deviceLabel("node")).toBe("API client");
  });

  it("falls back to deviceType, then a generic label, when there's nothing to parse", () => {
    expect(deviceLabel(undefined, "DESKTOP")).toBe("DESKTOP");
