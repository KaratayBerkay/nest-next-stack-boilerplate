...
export function createInviteSchema(t: Record<string, string>) {
  return z.object({
    emails: z.array(z.string().email(t.emailInvalid ?? "Invalid email")).min(1, t.emailRequired ?? "At least one email required"),
    role: z.enum(["member", "admin", "owner"], { message: t.roleRequired ?? "Select a role" }),
