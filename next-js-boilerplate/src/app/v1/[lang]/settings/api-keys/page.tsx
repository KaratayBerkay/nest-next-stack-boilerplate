...
export default function ApiKeysPage() {
  ...
  const loadKeys = useCallback(async () => {
    try {
      const data = await apiFetchJson<{ apiKeys: ApiKey[] }>("/api/api-keys");
  ...
  const handleCreate = async () => {
    ...
    try {
      const result = await apiFetchJson<CreateResult>("/api/api-keys", {
        method: "POST",
  ...
  const handleRevoke = async (id: string, name: string) => {
    ...
    try {
      await apiFetchJson(`/api/api-keys/${id}`, { method: "DELETE" });
  ...
  if (!user) return <UnauthenticatedMessage message="Sign in to manage API keys" />;
