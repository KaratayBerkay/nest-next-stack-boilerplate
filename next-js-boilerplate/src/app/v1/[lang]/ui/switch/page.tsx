    ...
    <div className="flex flex-col gap-4" data-testid="switch-demo">
      ...
      <section className="flex flex-col gap-3">
        ...
        <div className="surface max-w-sm divide-border divide-y overflow-hidden">
          ...
          <div className="flex items-center justify-between border-border border-t bg-transparent px-4 py-2">
            <span className="text-muted text-xs">
              {enabledCount} of 4 enabled
            </span>
            <Badge variant={enabledCount >= 3 ? "success" : "default"} className="text-[10px]">
              {enabledCount >= 3 ? "Mostly on" : enabledCount > 0 ? "Partial" : "All off"}
            </Badge>
          </div>
        </div>
      </section>
