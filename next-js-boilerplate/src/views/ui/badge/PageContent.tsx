    ...
    <div className="flex flex-col gap-6">
      ...
      <section className="flex flex-col gap-3">
        ...
        <div className="flex flex-col gap-3">
          <div className="surface flex items-start gap-3 rounded-lg border p-4 shadow-sm">
            <span className="mt-0.5">
              <Badge variant="success" size="sm">New</Badge>
            </span>
            <div className="flex-1">
              <p className="text-sm font-medium">Payment received</p>
              <p className="text-muted text-xs">$249.00 from Acme Corp</p>
            </div>
            <span className="text-muted text-xs">2m ago</span>
          </div>
          <div className="surface flex items-start gap-3 rounded-lg border p-4 shadow-sm">
            <span className="mt-0.5">
            ...
            <div className="flex-1">
              <p className="text-sm font-medium">Invoice due soon</p>
              <p className="text-muted text-xs">Invoice #1042 due in 3 days</p>
            </div>
            <span className="text-muted text-xs">1h ago</span>
          </div>
          <div className="surface flex items-start gap-3 rounded-lg border p-4 shadow-sm">
            <span className="mt-0.5">
            ...
            <div className="flex-1">
              <p className="text-sm font-medium">Subscription renewal failed</p>
              <p className="text-muted text-xs">Please update your payment method</p>
            </div>
            <span className="text-muted text-xs">3h ago</span>
          </div>
        </div>
      </section>
  ...
  return (
    <div className="flex flex-col gap-6">
      <section className="flex flex-col gap-3">
        ...
        <div className="flex flex-col gap-3">
          {users.map((user) => (
            <div key={user.name} className="surface flex items-center gap-3 rounded-lg border p-3 shadow-sm">
              <div className="relative">
                <div className="bg-surface flex size-10 items-center justify-center rounded-full text-sm font-medium">
                  {user.avatar}
                </div>
                <span className={cn("absolute -bottom-0.5 -right-0.5 size-3 rounded-full border-2 border-bg", statusColor(user.status))} />
              </div>
              <div className="flex-1">
      ...
      <section className="flex flex-col gap-3">
        ...
        <div className="surface rounded-xl border shadow-sm">
          <div className="grid grid-cols-4 border-b px-4 py-2 text-xs font-medium text-muted">
            <span>Order</span>
            <span>Status</span>
            <span>Amount</span>
            <span>Date</span>
          ...
          {orders.map((order) => (
            <div key={order.id} className="grid grid-cols-4 items-center border-b border-border/50 px-4 py-3 last:border-0">
