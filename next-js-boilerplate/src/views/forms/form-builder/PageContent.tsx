...
const RESERVED_NAMES = new Set(["__proto__", "constructor", "prototype", "id", "form"]);
...
const INITIAL_FIELDS: BuilderField[] = [
  { id: "1", type: "text", label: "Full Name", name: "fullName", required: true, options: [] },
  { id: "2", type: "select", label: "Country", name: "country", required: true, options: ["US", "Canada", "UK"] },
  { id: "3", type: "checkbox", label: "Newsletter", name: "newsletter", required: false, options: [] },
  { id: "4", type: "date", label: "Birth Date", name: "birthDate", required: false, options: [] },
];
...
export default function FormBuilderPage() {
  ...
  const handleAddField = useCallback(() => {
    ...
    setFields((prev) => [
      ...prev,
      { id: String(nextFieldId++), type: "text", label, name: sanitizeFieldName(label), required: false, options: [] },
    ]);
  }, []);
    ...
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Button size="sm" variant={preview ? "secondary" : "default"} onClick={() => setPreview(false)}>{t.formBuilder.builder}</Button>
      ...
      {preview ? (
        <div className="flex flex-col gap-4">
          <p className="text-xxs text-muted">Field names: {fields.map((f) => f.name).join(", ")}</p>
          {fields.map((field) => (
            <dynamicForm.AppField key={field.id} name={field.name}>
              {(f) => (
                <div className="flex flex-col gap-1">
                  ...
                  {field.type === "select" && (
                    <f.SelectField label={field.label} options={field.options.map((o) => ({ value: o, label: o }))} />
                  )}
                  {field.type === "checkbox" && <f.CheckboxField label={field.label} options={[{ value: "true", label: field.label }]} />}
                  {field.type === "date" && <f.DateField label={field.label} />}
                </div>
              )}
            </dynamicForm.AppField>
          ))}
          <Button onClick={handlePreviewSubmit}>{t.formBuilder.submitPreview}</Button>
        </div>
      ) : (
        <div className="flex flex-col gap-3">
          {fields.map((field, idx) => (
            <div key={field.id} className="surface flex flex-col gap-2 rounded-lg border border-border p-3">
              <div className="flex items-center justify-between">
                <span className="text-xs font-medium">{field.label || "Untitled"}</span>
                <div className="flex gap-1">
                  <Button size="sm" variant="ghost" disabled={idx === 0} onClick={() => handleMoveUp(idx)}>{t.formBuilder.moveUp}</Button>
                  <Button size="sm" variant="ghost" disabled={idx >= fields.length - 1} onClick={() => handleMoveDown(idx)}>{t.formBuilder.moveDown}</Button>
                  <Button size="sm" variant="destructive" onClick={() => handleRemoveField(field.id)}>{t.formBuilder.removeField}</Button>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-2">
                <div className="flex flex-col gap-1">
                  ...
                  <select
                    ...
                    value={field.type}
                    onChange={(e) => handleUpdateField(field.id, { type: e.target.value as BuilderField["type"] })}
                  >
                    {FIELD_TYPES.map((ft) => <option key={ft.value} value={ft.value}>{ft.label}</option>)}
                  </select>
                </div>
                <div className="flex flex-col gap-1">
                  <span className="text-xxs text-muted">{t.formBuilder.fieldLabel}</span>
                  <input
                    className="rounded border border-border bg-field px-2 py-1 text-xs"
                    value={field.label}
              ...
              {field.type === "select" && (
                <div className="flex flex-col gap-1">
                  <span className="text-xxs text-muted">{t.formBuilder.fieldOptions}</span>
                  <input
                    className="rounded border border-border bg-field px-2 py-1 text-xs"
