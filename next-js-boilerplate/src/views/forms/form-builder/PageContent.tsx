...
import {
  ...
  getNextFieldId,
  INITIAL_FIELDS,
} from "@/views/forms/form-builder/FormBuilder-utils";
        ...
        prev.map((f) => {
          ...
          if (patch.label && patch.label !== f.label) {
            const otherNames = prev
              .filter((x) => x.id !== id)
              .map((x) => x.name);
            updated.name = sanitizeFieldName(patch.label, otherNames);
          }
          return updated;
        }),
      );
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {preview ? (
        <FormPreview
          fields={fields}
          dynamicForm={dynamicForm as unknown as ReturnType<typeof useAppForm>}
