...
const BADGE_COLORS: Record<GalleryMode, string> = {
  ...
  simulated: "bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400",
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 md:grid-cols-3">
        {examples.map((ex) => (
          <Link
            ...
            className="surface flex flex-col gap-2 rounded-lg border border-border p-4 transition-colors hover:bg-surface-hover"
          >
            <div className="flex items-start justify-between gap-2">
              <span className="text-sm font-medium">{ex.name}</span>
              <span
                className={`shrink-0 rounded px-1.5 py-0.5 text-xxs font-medium ${BADGE_COLORS[ex.mode]}`}
              >
                {t.badge[ex.mode]}
              </span>
            </div>
