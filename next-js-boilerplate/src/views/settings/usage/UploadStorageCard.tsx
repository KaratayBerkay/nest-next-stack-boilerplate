...
import { storageUsageQueryOptions } from "@/api/client/usage/query";
...
export function UploadStorageCard({ className }: ClassNameProps) {
  const t = useMessages("settings");
  const { data, isLoading } = useQuery(storageUsageQueryOptions());
  ...
  return (
    <Card variant="surface" className={cn("p-5", className)}>
      <div className="flex flex-col gap-1.5">
        <Label>{t.uploadStorageTitle}</Label>
        ...
        <p className="text-muted text-sm">
          {t.uploadStorageFiles.replace("{count}", String(data.fileCount))} ·{" "}
          {t.uploadStorageMultiplier.replace(
            "{count}",
      ...
      {overLimit && (
        <p className="text-error mt-3 text-sm">{t.uploadStorageLimitReached}</p>
      )}

      <p className="text-muted mt-3 text-xs">{t.uploadStorageUpgradeHint}</p>
    </Card>
  );
}
