...
export function StepWizardOnboarding() {
  ...
  const teamSizeLabel = TEAM_SIZES.find(
    (size) => size.value === teamSize,
  )?.labelKey;
  const previewName =
    workspaceName.trim() || ob.onboarding1PreviewNamePlaceholder;
        ...
        <div className="border-border bg-surface grid overflow-hidden rounded-2xl border lg:grid-cols-[0.9fr_1.3fr]">
          <div className="bg-muted/40 border-border flex flex-col gap-5 border-b p-8 lg:border-r lg:border-b-0">
            ...
            <div className="flex flex-col gap-1">
              ...
              <span className="text-fg text-lg font-semibold break-words">
                {previewName}
              </span>
            </div>
          ...
          <div className="flex flex-col gap-6 p-8">
              ...
              <div className="flex flex-col gap-5">
                <div className="flex flex-col gap-1">
                  <h3 className="text-fg text-lg font-semibold">
                    {ob.onboarding1Step1Title}
                  </h3>
                  <p className="text-muted text-sm">
                    {ob.onboarding1Step1Subtitle}
                  </p>
                </div>
                <div className="flex flex-col gap-1.5">
                  <Label htmlFor="onboarding1-name">
                    {ob.onboarding1WorkspaceNameLabel}
                  </Label>
                  <Input
                    id="onboarding1-name"
                    type="text"
                  ...
                  >
                    {TEAM_SIZES.map((size) => (
                      ...
                      >
                        <RadioGroupItem
                          value={size.value}
                          id={`onboarding1-size-${size.value}`}
                        />
            ...
            {step === 2 && (
              <div className="flex flex-col gap-4">
                <div className="flex flex-col gap-1">
                  <h3 className="text-fg text-lg font-semibold">
                    {ob.onboarding1Step2Title}
                  </h3>
                  <p className="text-muted text-sm">
                    {ob.onboarding1Step2Subtitle}
                  </p>
                </div>
            ...
            {step === 3 && (
              <div className="flex flex-col gap-4">
                <div className="flex flex-col gap-1">
                  <h3 className="text-fg text-lg font-semibold">
                    {ob.onboarding1Step3Title}
                  </h3>
                  <p className="text-muted text-sm">
                    {ob.onboarding1Step3Subtitle}
                  </p>
                </div>
                <div className="flex flex-col gap-1.5">
                  <Label htmlFor="onboarding1-email">
                    {ob.onboarding1EmailLabel}
                  </Label>
            ...
            {step === 4 && (
              <div className="flex flex-col items-center gap-4 py-4 text-center">
                ...
                <ul className="text-fg flex w-full flex-col gap-2 text-left text-sm">
                  ...
                  <li className="border-border rounded-lg border px-3 py-2">
                    {ob.onboarding1SummaryFocus.replace(
                      "{count}",
                      String(focusAreas.length),
                    )}
                  </li>
                  <li className="border-border rounded-lg border px-3 py-2">
                    {ob.onboarding1SummaryInvites.replace(
                      "{count}",
