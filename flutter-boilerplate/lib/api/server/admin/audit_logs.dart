...
class AuditLogEntry {
  ...
  factory AuditLogEntry.fromJson(Map<String, dynamic> json) {
    return AuditLogEntry(
      ...
      actor: json['actor'] != null ? AuditActor.fromJson(json['actor'] as Map<String, dynamic>) : null,
...
class AuditLogResponse {
  ...
  const AuditLogResponse({required this.items, required this.total, required this.take, required this.skip});

  factory AuditLogResponse.fromJson(Map<String, dynamic> json) {
    return AuditLogResponse(
      items: (json['items'] as List<dynamic>?)?.map((e) => AuditLogEntry.fromJson(e as Map<String, dynamic>)).toList() ?? [],
...
final auditLogsServerProvider = Provider((ref) => AuditLogsServer(ref.read(dioProvider)));
...
class AuditLogsServer {
  ...
  Future<AuditLogResponse> call([AuditLogParams? params]) async {
    final queryParams = params?.toQueryParams() ?? const AuditLogParams().toQueryParams();
    final response = await _dio.get<dynamic>(Urls.adminAuditLogs, queryParameters: queryParams);
    if (response.data is List) {
      final items = (response.data as List).map((e) => AuditLogEntry.fromJson(e as Map<String, dynamic>)).toList();
      return AuditLogResponse(items: items, total: items.length, take: 50, skip: 0);
    }
