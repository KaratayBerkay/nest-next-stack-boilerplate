...
import { useStreamViewers } from "@/hooks/rtc/useStreamViewers";
import { StreamPlayer } from "@/components/rtc/StreamPlayer";
import { StreamChatPanel } from "@/components/rtc/StreamChatPanel";
import { StreamViewerList } from "@/components/rtc/StreamViewerList";
...
export function RtcLiveViewerView() {
  ...
  // "" until the join lands — the hook's own guards keep the disabled query
  // from fetching for a stream this user can't see yet.
  const viewers = useStreamViewers(phase === "active" ? slug : "");
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden bg-neutral-950">
      <div className="flex flex-1 overflow-hidden">
        {/* Fixed-fit column, same shape as RtcGoLiveView: the player flexes
            to the leftover height, the stream info bar stays pinned below —
            nothing scrolls. */}
        <div className="flex min-w-0 flex-1 flex-col overflow-hidden">
          <div className="min-h-0 flex-1">
            <StreamPlayer
              ...
              micMuted={
                livekit.broadcasterOnline && !livekit.broadcasterMicEnabled
              }
