...
export function PackageBuilderServices() {
  ...
  const s = t.services;
  const [selected, setSelected] = useState<string[]>([SERVICES[0].id, SERVICES[2].id]);
    ...
    <section className="w-full py-16 lg:py-24">
      <div className="mx-auto max-w-5xl px-6 lg:px-8">
        <div className="flex flex-col gap-4">
          ...
          <p className="text-muted max-w-xl leading-relaxed">{s.services8Intro}</p>
        </div>
                  ...
                  <>
                    ...
                    <div className="border-border mt-4 flex items-baseline justify-between border-t pt-4">
                      ...
                      <span className="text-fg text-2xl font-semibold tracking-tight tabular-nums">
                        {s.services8TotalTemplate.replace("{amount}", String(total))}
                      </span>
                    </div>
                  </>
                )}
...
function PackageRow({ checked, onToggle, name, description, priceLabel }: PackageRowProps) {
  const inputId = useId();
