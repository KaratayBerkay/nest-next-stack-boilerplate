...
const INITIAL_STATE: Record<string, CellState> = Object.fromEntries(
  ROWS.map((row) => [row.id, { email: row.defaultEmail, push: row.defaultPush }]),
);
...
export function ChannelMatrixTableSettingsNotifications() {
  ...
  const [values, setValues] = useState<Record<string, CellState>>(INITIAL_STATE);
    ...
    <section className="w-full py-16 lg:py-24">
      <div className="mx-auto max-w-3xl px-6 lg:px-8">
        <div className="flex flex-col gap-1 pb-6">
          ...
          <p className="text-muted text-sm">{sn.settingsNotifications4Subheading}</p>
        </div>
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableRow key={row.id}>
                <TableCell>
                  <div className="flex flex-col gap-0.5">
                    ...
                    <span className="text-muted text-xs">{sn[row.descKey]}</span>
                  </div>
                </TableCell>
                <TableCell>
                  <div className="flex justify-center">
                    <Checkbox
                      ...
                      onChange={(e) => toggleCell(row.id, "email", e.target.checked)}
                ...
                <TableCell>
                  <div className="flex justify-center">
                    <Checkbox
                      ...
                      onChange={(e) => toggleCell(row.id, "push", e.target.checked)}
