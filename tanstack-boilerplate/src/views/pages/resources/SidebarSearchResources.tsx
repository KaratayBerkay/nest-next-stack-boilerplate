...
const CATEGORIES: CategoryDef[] = [
  ...
  { value: "getting-started", labelKey: "resources2Category1Label", icon: IconRocket },
  { value: "guides", labelKey: "resources2Category2Label", icon: IconBook2 },
  { value: "integrations", labelKey: "resources2Category3Label", icon: IconPlugConnected },
  { value: "case-studies", labelKey: "resources2Category4Label", icon: IconBriefcase },
  { value: "release-notes", labelKey: "resources2Category5Label", icon: IconGitBranch },
];
...
function countByCategory(items: readonly ResourceRow[]): Map<FilterValue, number> {
    ...
    <section className="w-full py-16 lg:py-24">
      <div className="mx-auto max-w-6xl px-6 lg:px-8">
        <div className="flex max-w-2xl flex-col gap-3">
          ...
          <p className="text-muted leading-relaxed">{r.resources2Description}</p>
        </div>
        ...
        <div className="mt-8 grid grid-cols-1 gap-8 lg:grid-cols-[240px_1fr]">
          <aside className="flex flex-col gap-4">
            ...
            />
            <nav aria-label={r.resources2Eyebrow} className="flex flex-col gap-1">
              {CATEGORIES.map((cat) => {
          ...
          <div className="flex flex-col gap-4">
            <span className="text-muted text-xs">
              {r.resources2ResultsCount.replace("{count}", String(filtered.length))}
            </span>
            {filtered.length === 0 ? (
              <Empty
              ...
              <ul className="border-border divide-border overflow-hidden rounded-2xl border divide-y">
                {filtered.map((item) => (
                  <li key={item.id}>
                    <Link
                      href={LINK_URL}
