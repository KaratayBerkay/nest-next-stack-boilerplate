...
const RECORDS = [
  { id: "a", src: "/img/placeholders/ph-1x1-2.webp", nameKey: "fieldMapping2RecordAName", emailKey: "fieldMapping2RecordAEmail" },
  { id: "b", src: "/img/placeholders/ph-1x1-5.webp", nameKey: "fieldMapping2RecordBName", emailKey: "fieldMapping2RecordBEmail" },
] as const;

const CONFLICT_FIELDS = [
  { id: "phone", labelKey: "fieldMapping2FieldPhone", valueAKey: "fieldMapping2PhoneA", valueBKey: "fieldMapping2PhoneB" },
  { id: "company", labelKey: "fieldMapping2FieldCompany", valueAKey: "fieldMapping2CompanyA", valueBKey: "fieldMapping2CompanyB" },
  { id: "role", labelKey: "fieldMapping2FieldRole", valueAKey: "fieldMapping2RoleA", valueBKey: "fieldMapping2RoleB" },
] as const;
        ...
        <Dialog>
          ...
          <DialogContent className="max-w-lg">
            <DialogHeader>
              ...
              <DialogDescription>{fm.fieldMapping2Description}</DialogDescription>
            </DialogHeader>
              ...
              <div className="grid grid-cols-2 gap-3">
                {RECORDS.map((record) => (
                  ...
                  >
                    ...
                    />
                    <span className="text-fg text-sm font-semibold">{fm[record.nameKey]}</span>
                    <span className="text-muted text-xs">{fm[record.emailKey]}</span>
                  </div>
                ))}
              </div>
                ...
                {CONFLICT_FIELDS.map((field) => (
                  <div key={field.id} className="flex flex-col gap-1.5">
                    ...
                    <RadioGroup
                      ...
                      onValueChange={(value) =>
                        setPicks((current) => ({ ...current, [field.id]: value as "a" | "b" }))
                      }
                    ...
                    >
                      ...
                      >
                        ...
                        <span className="truncate text-sm">{fm[field.valueAKey]}</span>
                      </label>
                      <label
                      ...
                      >
                        ...
                        <span className="truncate text-sm">{fm[field.valueBKey]}</span>
                      </label>
                    </RadioGroup>
                  </div>
                ))}
            ...
            <DialogFooter>
              <DialogClose variant="ghost">{fm.fieldMapping2Cancel}</DialogClose>
