...
const SECTIONS: SectionEntry[] = [
  { id: "introduction", titleKey: "sidebar8Section1Title", bodyKey: "sidebar8Section1Body" },
  { id: "installation", titleKey: "sidebar8Section2Title", bodyKey: "sidebar8Section2Body" },
  { id: "configuration", titleKey: "sidebar8Section3Title", bodyKey: "sidebar8Section3Body" },
  { id: "deployment", titleKey: "sidebar8Section4Title", bodyKey: "sidebar8Section4Body" },
];
...
export function DocsTocScrollSidebar() {
  ...
  const handleScroll = () => {
    ...
    setProgress(
      scrollableHeight > 0
        ? Math.min(100, Math.round((container.scrollTop / scrollableHeight) * 100))
        : 0,
    );
    ...
    for (const section of SECTIONS) {
      ...
      if (el && el.offsetTop - container.offsetTop <= container.scrollTop + 24) {
        current = section.id;
      }
    }
    setActiveId(current);
        ...
        <div className="bg-surface border-border flex h-[560px] w-full overflow-hidden rounded-2xl border">
          <aside className="border-border bg-surface flex w-60 shrink-0 flex-col border-r p-4">
            ...
            <div className="mt-auto flex flex-col gap-1 pt-4">
              <span className="text-muted text-xs">{sb.sidebar8ProgressLabel}</span>
              <Progress value={progress} size="sm" aria-label={sb.sidebar8ProgressLabel} />
            </div>
          </aside>
