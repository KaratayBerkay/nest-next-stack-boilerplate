  ...
  return (
    <section className="w-full py-16 lg:py-24">
      <div className="mx-auto flex max-w-4xl flex-col px-6 lg:px-8">
        ...
        <div className="border-border divide-border border-t divide-y">
          {ROWS.map((row) => (
            <Link
              key={row.id}
              href={LINK_URL}
              className="group flex items-center justify-between gap-6 py-5 transition-colors hover:bg-surface-hover sm:py-6"
            >
              ...
              <div className="flex shrink-0 items-center gap-4">
                ...
                <IconArrowUpRight
                  size={18}
                  className="text-muted shrink-0 transition-transform duration-200 group-hover:-translate-y-0.5 group-hover:translate-x-0.5 group-hover:text-fg"
                  aria-hidden="true"
                />
              </div>
            </Link>
