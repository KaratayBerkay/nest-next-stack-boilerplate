...
const RANK_BADGE_VARIANT: Record<AwardRank, "default" | "secondary" | "outline"> = {
  winner: "default",
  finalist: "secondary",
  ...
  return (
    <section className="w-full py-16 lg:py-24">
      <div className="mx-auto flex max-w-4xl flex-col px-6 lg:px-8">
        ...
        <div className="border-border bg-surface divide-border overflow-hidden rounded-2xl border divide-y">
          {ROWS.map((row) => (
            <div key={row.id} className="flex items-center gap-4 p-5 sm:p-6">
              ...
              <div className="flex min-w-0 flex-1 flex-col gap-1">
                ...
                <span className="text-muted flex items-center gap-1.5 text-sm">
                  <IconCalendar size={14} className="shrink-0" aria-hidden="true" />
                  <span className="truncate">
                    {a[row.orgNameKey]} · {a[row.dateKey]}
                  </span>
                </span>
              </div>
              <Badge variant={RANK_BADGE_VARIANT[row.rank]} size="sm" className="shrink-0">
                {a[row.rankKey]}
              </Badge>
            </div>
          ))}
