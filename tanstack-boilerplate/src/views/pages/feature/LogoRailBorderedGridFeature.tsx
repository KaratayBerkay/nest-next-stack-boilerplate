...
const CELLS = [
  { id: "cell1", titleKey: "feature161Cell1Title", bodyKey: "feature161Cell1Body" },
  { id: "cell2", titleKey: "feature161Cell2Title", bodyKey: "feature161Cell2Body" },
] as const;
        ...
        <div className="border-border mt-12 grid gap-px overflow-hidden rounded-xl border bg-transparent sm:grid-cols-2">
          {CELLS.map((cell) => (
            <div key={cell.id} className="bg-surface flex flex-col gap-4 p-6">
              ...
              <h3 className="text-fg text-base font-semibold">{f[cell.titleKey]}</h3>
              <p className="text-muted text-sm leading-relaxed">{f[cell.bodyKey]}</p>
            </div>
          ))}
        </div>
      </div>
