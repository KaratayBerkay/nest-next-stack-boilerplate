          ...
          {SUPPORT_ITEMS.map((item) => (
            <div key={item.id} className="flex items-center gap-3">
              ...
              <div className="flex flex-col">
                <span className="text-fg text-sm font-medium">{f[item.labelKey]}</span>
      ...
      <div className="mx-auto max-w-6xl px-6 py-10 lg:px-8">
          ...
          {GROUPS.map((group) => (
            <AccordionItem key={group.id} value={group.id}>
              <AccordionTrigger className="group">
                <span className="text-fg text-sm font-semibold">{f[group.titleKey]}</span>
                <IconChevronDown
                  size={16}
        ...
        <div className="bg-brand/10 border-brand/30 mt-10 flex flex-col items-start gap-4 rounded-2xl border p-8 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex flex-col gap-1">
            ...
            <p className="text-muted max-w-md text-sm">{f.ecommerceFooter19PanelCopy}</p>
          </div>
          <Button
            variant="primary"
        ...
        <p className="text-muted mt-8 text-xs">{f.ecommerceFooter19Copyright}</p>
      </div>
    </footer>
  );
}
