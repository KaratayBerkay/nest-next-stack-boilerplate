...
function makeRequest(method: string, path: string) {
  return new NextRequest(`http://localhost/api/rest/${path}`, {
    method,
    ...(method === "POST"
...
describe("BFF /api/rest/[...path]", () => {
  ...
  it("GET never needs a CSRF token", async () => {
    ...
    const res = await GET(makeRequest("GET", "reactions"), {
      params: Promise.resolve({ path: ["reactions"] }),
    });
  ...
  it("POST rejects the same traversal segment with 400 before even checking CSRF", async () => {
    ...
    const res = await POST(makeRequest("POST", "reactions"), {
