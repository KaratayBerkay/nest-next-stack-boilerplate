...
// Deliberately not part of STREAM_FIELDS: the list is only wanted by the
// go-live/viewer pages' sidebar, and pulling it into every stream query
// would make the discovery list resolve viewers per row.
export const STREAM_VIEWERS_QUERY = `
  query StreamViewers($slug: String!) {
    streamBySlug(slug: $slug) {
      viewers {
        userId
        name
