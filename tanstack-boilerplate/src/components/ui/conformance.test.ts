...
const demoBase = join(srcDir, "routes/v1/$lang/ui");
...
describe("UI component conformance", () => {
  for (const folder of folders) {
    describe(folder, () => {
      ...
      it("has a demo route in routes/v1/$lang/ui/", () => {
        expect(existsSync(join(demoBase, folder, "index.tsx"))).toBe(true);
      });
    });
  }
});
...
describe("Registry completeness", () => {
  it("every registry slug has a demo route", () => {
    ...
    for (const slug of registry) {
      if (!existsSync(join(demoBase, slug, "index.tsx"))) {
        missing.push(slug);
      }
    }
    expect(
