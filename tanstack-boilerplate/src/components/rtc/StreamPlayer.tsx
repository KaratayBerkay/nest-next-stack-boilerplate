...
export function StreamPlayer({
  ...
  // Fills whatever box the view gives it (both stream views size it with a
  // flex-1/min-h-0 wrapper so the page fits the viewport with no scrolling);
  // the video letterboxes itself via object-contain. The old aspect-video
  // root made the stage ~9/16 of a wide window's width — taller than the
  // viewport — and forced the whole column to scroll.
  return (
    <div className="bg-overlay relative flex h-full w-full items-center justify-center overflow-hidden rounded-t-lg">
      {activeVideoTrack ? (
      ...
      {/* Broadcaster muted their mic — same badge treatment as the meeting
          participant tiles, distinct from the no-audio-track indicator. */}
      {micMuted && (
        <span className="bg-overlay/60 text-error absolute top-3 right-3 flex size-7 items-center justify-center rounded-full backdrop-blur-sm">
