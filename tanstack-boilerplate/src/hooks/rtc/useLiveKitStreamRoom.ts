...
export interface UseLiveKitStreamRoomResult {
  ...
  /** False while the broadcaster's mic is muted — drives the on-player mute
   *  badge. True when no broadcaster is online (no badge to show). */
  broadcasterMicEnabled: boolean;
...
export function useLiveKitStreamRoom(
  ...
  const [broadcasterMicEnabled, setBroadcasterMicEnabled] = useState(true);
  ...
  const rebuildBroadcasterTracks = useCallback(() => {
    ...
    if (!broadcaster) {
      ...
      setScreenShareTrack(null);
      setAudioTrack(null);
      setBroadcasterMicEnabled(true);
      return;
    }
    setBroadcasterOnline(true);
    setBroadcasterMicEnabled(broadcaster.isMicrophoneEnabled);
  ...
  // Un-pause the stream video when the tab returns to the foreground —
  // without this, mobile playback stays black after backgrounding.
  useResumeMediaOnForeground(roomRef, connected, rebuildBroadcasterTracks);
