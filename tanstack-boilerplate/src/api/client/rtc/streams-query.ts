...
export function streamViewersQueryOptions(slug: string) {
  return queryOptions({
    queryKey: ["rtc", "streams", slug, "viewers"],
    queryFn: async () => {
      const { getStreamViewersServer } =
        await import("@/api/server/rtc/streams/viewers");
      return getStreamViewersServer(slug);
    },
    enabled: Boolean(slug),
  });
}
